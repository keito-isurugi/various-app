/**
 * src/app/algorithms/insertion-sort/page.tsx
 *
 * 挿入ソートアルゴリズムの解説ページ
...
import { insertionSortExplanation } from "../../../data/explanations/insertion-sort-explanation";
import type { AlgorithmInput, AlgorithmResult } from "../../../types/algorithm";
import { InsertionSortAlgorithm } from "../../../utils/algorithms/insertion-sort";
...
export default function InsertionSortPage() {
	// アルゴリズムインスタンス
	const algorithm = new InsertionSortAlgorithm();
	...
	const generateRandomArray = useCallback(() => {
		...
		const array = InsertionSortAlgorithm.generateRandomArray(size, 50);
	...
	const generateReverseArray = useCallback(() => {
		...
		const array = InsertionSortAlgorithm.generateReverseArray(size);
	...
	const generateSortedArray = useCallback(() => {
		const size = Math.floor(Math.random() * 4) + 5; // 5-8個の要素
		const array = InsertionSortAlgorithm.generateSortedArray(size);
	...
	/**
	 * ほぼソート済み配列を生成（挿入ソートが得意なケース）
	 */
	const generateNearlySortedArray = useCallback(() => {
		const size = Math.floor(Math.random() * 4) + 6; // 6-9個の要素
		const array = InsertionSortAlgorithm.generateNearlySortedArray(size);
			...
			<div className="container mx-auto px-4 py-8 max-w-6xl">
				...
				<section className="mt-12">
					<CalculationExplanation
						explanationData={insertionSortExplanation}
...
console.log(sortedArray); // [1, 2, 3, 4, 5, 6]`}</code>
							</pre>
						</div>
					</div>
				</section>
