...
describe("ParameterInput", () => {
	...
	describe("表示", () => {
		it("パラメータ名が表示される", () => {
			render(<ParameterInput parameter={defaultParameter} onChange={mockOnChange} />);
		...
		it("必須でない場合は必須マークが表示されない", () => {
			...
			render(<ParameterInput parameter={optionalParameter} onChange={mockOnChange} />);
	...
	describe("入力操作", () => {
		...
		it("無効な数値入力でエラーが表示される", () => {
			...
			expect(screen.getByText("有効な数値を入力してください")).toBeInTheDocument();
		...
		it("最小値未満の入力でエラーが表示される", () => {
			...
			expect(screen.getByText("値は0以上である必要があります")).toBeInTheDocument();
		});
		...
		it("最大値超過の入力でエラーが表示される", () => {
			...
			expect(screen.getByText("値は1e+50以下である必要があります")).toBeInTheDocument();
		});
	});
