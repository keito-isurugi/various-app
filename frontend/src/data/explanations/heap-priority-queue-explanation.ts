	...
	sections: [
		{
			...
			title: " ヒープの基本構造",
			content:
				"ヒープは完全二分木の性質を持つ特殊なデータ構造です。完全二分木とは、最下段を除く全ての段が完全に埋まっており、最下段は左から順に埋まっている木構造を指します。この性質により、配列による効率的な実装が可能になり、親子関係を単純な計算で求めることができます。ヒープには最大ヒープと最小ヒープの2種類があり、最大ヒープでは親ノードが子ノードより大きく、最小ヒープでは親ノードが子ノードより小さくなります。配列実装では、インデックスiの要素に対して、左の子は2i+1、右の子は2i+2、親はfloor((i-1)/2)の位置にあります。",
			importance: "high",
			examples: [
			...
			formula:
				"完全二分木の性質:<br>• 左の子: 2i + 1<br>• 右の子: 2i + 2<br>• 親: floor((i-1)/2)<br><br>ヒープ性質:<br>• 最大ヒープ: parent ≥ children<br>• 最小ヒープ: parent ≤ children",
		},
...

		{
			...
			title: "【解析】配列による効率的実装",
			content:
				"ヒープの配列実装は、完全二分木の性質を活用した非常に効率的な方法です。配列のインデックス0から開始する場合、インデックスiの要素に対して、左の子は2i+1、右の子は2i+2、親はfloor((i-1)/2)で計算できます。この実装により、ポインタを使わずに木構造を表現でき、メモリ効率が向上します。また、キャッシュ効率も良く、実際の性能が理論値に近い値を示します。配列実装では、動的なメモリ確保が不要で、連続したメモリ領域を使用するため、メモリ断片化の心配もありません。さらに、親子関係の計算が単純な算術演算で済むため、実装が簡潔になり、バグの混入も少なくなります。",
			importance: "medium",
			examples: [
				"インデックス計算: 親floor((i-1)/2), 左子2i+1, 右子2i+2",
				"メモリ効率: ポインタ不要、連続領域使用",
		...
		{
			id: "build-heap",
			title: "【構造】ヒープ構築アルゴリズム",
			content:
				"配列からヒープを構築する際には、ボトムアップ方式を使用することで線形時間O(n)での構築が可能です。この方法では、最後の非葉ノード（インデックスfloor(n/2)-1）から開始して、各ノードに対してbubble down操作を実行します。葉ノードは既にヒープ性質を満たしているため、内部ノードのみを処理すれば十分です。なぜO(n)で構築できるかというと、各レベルでの操作回数と移動距離を考慮すると、全体として線形時間に収束するためです。具体的には、最下位レベルの要素は移動しない、その上のレベルは最大1回移動、さらに上は最大2回移動となり、幾何級数的に減少します。この効率的な構築アルゴリズムにより、大量のデータからのヒープ生成が実用的な時間で完了します。",
			importance: "medium",
			examples: [
				"開始位置: 最後の非葉ノードfloor(n/2)-1",
				"処理順序: 右から左、下から上",
				"各ノード: bubble down操作を実行",
				"計算量解析: Σ(h × 2^h) = O(n)",
			],
			formula:
				"構築アルゴリズム:<br>for i = floor(n/2)-1 down to 0:<br>&nbsp;&nbsp;heapify(i)<br><br>時間計算量証明:<br>Σ(h=0 to log n) h × 2^h ≈ 2n = O(n)",
		},

		{
			id: "priority-queue-applications",
			title: "【ポイント】優先度付きキューとしての応用",
			content:
